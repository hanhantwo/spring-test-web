...
MySQL的表级锁有两种模式：**表共享读锁（Table Read Lock）锁住一张表时其他表也不能读**和**表独占写锁（Table Write Lock）**。 
** 共享读锁在锁住一张表时查询其他表时也不能查询。为了防止死锁。 A表可能关联B表字段，
...
|                           session1                           |                           session2                           |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| set autocommit=0（设置开启事物）<br />
  select * from tab_no_index where id = 1; | set autocommit=0<br />select * from tab_no_index where id =2 |
