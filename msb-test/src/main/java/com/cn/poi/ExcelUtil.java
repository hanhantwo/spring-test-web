...
import com.cn.poi.annotation.Excel;
import com.cn.poi.annotation.Excels;
...
public class ExcelUtil<T> {
	...
	public List<T> importExcel(InputStream is) throws Exception {
		return this.importExcel(StringUtils.EMPTY, is);
	}
...
//		return this.exportExcel();
	...
	public void exportExcel() {
		...
		try {
			...
			wb.write(out);
//			return GenericResult.success(filename);
		} catch (Exception e) {
	...
	public void setDataValidation(Excel attr, Row row, int column) {
		...
		if (StringUtils.isNotEmpty(attr.prompt())) {
			// 这里默认设了2-101列提示.
			this.setXSSFPrompt(sheet, "", attr.prompt(), 1, 100, column, column);
		}
		// 如果设置了combo属性则本列只能选择不能输入
		if (attr.combo().length > 0) {
			// 这里默认设了2-101列只能选择不能输入.
			this.setXSSFValidation(sheet, attr.combo(), 1, 100, column, column);
		}
	}
	 ...
	 * @return 结果
	 */
	public void exportExcel2(List<T> list, String sheetName, HttpServletResponse response) {
		this.init(list, sheetName, Excel.Type.EXPORT);
		this.exportExcel2(response);
	}
}